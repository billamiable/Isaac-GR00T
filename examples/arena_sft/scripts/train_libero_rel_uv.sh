#!/usr/bin/env bash
# LIBERO Franka task-space relative xyz+rotvec GR00T N1.6 SFT.
#
# Required environment variables:
#   BASE_MODEL_PATH  Local GR00T-N1.6-3B checkpoint or Hugging Face model ID.
#   DATASET_PATH     all_libero_suites_rel_rotvec LeRobot dataset root.
...
: "${DATASET_PATH:?Set DATASET_PATH to all_libero_suites_rel_rotvec}"
: "${OUTPUT_DIR:?Set OUTPUT_DIR to a writable checkpoint directory}"

MODALITY_CONFIG_PATH="${MODALITY_CONFIG_PATH:-${ARENA_SFT_ROOT}/configs/libero_franka_task_space_rel_n16_config.py}"
NUM_GPUS="${NUM_GPUS:-8}"
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-160}"
